...
struct Response *ftp_handle_http_request_with_options(struct Request *req, const char *root_dir, const char *css_path) {
	...
	// Handle style.css
	if (ends_with(decoded_path, "/style.css") || strcmp(decoded_path, "style.css") == 0) {
		const char *actual_css_path = (css_path && css_path[0] != '\0') ? css_path : "style.css";
		...
		char *css_content = malloc(fsize + 1);
		if (!css_content) {
			fclose(f);
		...
		struct Response *res = create_http_response(200, css_content);
		free(css_content); // create_http_response strdups the body
		map_put(res->headers, "Content-Type", "text/css");
		return res;
	}
	...
	if (strcmp(base_dir, ".") == 0) {
		snprintf(full_path, sizeof(full_path), "%s", rel_path[0] ? rel_path : ".");
	} else {
	...
	if (S_ISDIR(st.st_mode)) {
		struct StringBuilder *sb = sb_create(4096);
		...
		if (d) {
			...
			while ((dir = readdir(d)) != NULL) {
				...
				snprintf(entry_path, sizeof(entry_path), "%s/%s", full_path, dir->d_name);
				...
				if (strcmp(decoded_path, "/") == 0)
					...
					snprintf(href, sizeof(href), "%s/%s", decoded_path, dir->d_name);
		...
		struct Response *res = create_http_response(200, html);
		free(html);
		...
		struct Response *res = create_http_response(200, content);
		free(content);
		map_put(res->headers, "Content-Type", "application/octet-stream");
		return res;
	}
}
